...
После импорта транзакций на сервер рекомендуется сделать экспорт операций с сервера, чтобы убедиться, что операция прошла успешно. Автоматически этого не происходит потому, что вызов функции эжкспорта очищает поле "Изменено" и любые изменения в файле данных, которые не были переданы на сервер.
