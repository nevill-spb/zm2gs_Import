...
Есть три режима работы - Сохранить, Частично и Заменить. "Сохранить" обрабатывает все записи на листе счетов (добавляет новые, если ID нет в списке текущих; меняет существующие; удаляет счета, отмеченные флагом "Удалить"), "Частично" - только отмеченные флагом "Изменить". А вот функция "Заменить" пытается взять структуру с листа счетов, и привести счета на сервере в соответствие с ней, т.е. воссоздать счета с листа, которых нет на сервере, и удалить с сервера те счета, которых нет на листе. Функция "Заменить" определяет, существует счёт или нет, по его ID - т.е. счета с одинаковым названием, но с разными ID для неё разные.
