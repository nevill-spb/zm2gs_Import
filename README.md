...
ВАЖНО. Модули постоянно обновляются, когда у меня доходят руки что-то поменять. Выкачивайте модули одновременно. Иначе выкачав часть скриптов, а через неделю решив добавить к ним ещё пару модулей, вы обнаружите, что новые модули не работают, т.к. ссылаются на более современную версию предыдущих скриптов.

Обновите файл таблиц. Если появилось меню "Дзен Мани", поздравляю, у вас получилось. Запустите любой пункт меню, чтобы появился запрос на предоставление скриптам необходимых прав, и подтвердите его. Запустите Дзен Мани -> Полная Синхронизация, чтобы проверить работу скриптов по экспорту данных из Дзен Мани. Выберите пользователя и валюту по умолчанию на листе настроек. Выберите, как производить обработку нескольких категорий - перечислять их в одной строке с разделителями, или же выделять под каждую отдельный столбец. Вы готовы приступить к работе! 
...
* HTML-файл [CategoryReplacementDialog](https://github.com/nevill-spb/zm2gs_Import/blob/main/Scripts/CategoryReplacementDialog.html) для обработки замены одной категории на другую
...
Скрипт Main добавляет в Google Таблицы меню "Дзен Мани", которое содержит следующие кнопки, в зависимости от установленных вами скриптов:
* Полная Синхронизация: последовательно вызывает скрипты синхронизации и экспорта транзакций, словарей, категорий, счетов и мест на отдельные листы. Включает обновление словарей
* Обновить Словари: обновляет только данные словарей и записывает их на отдельный лист

Подменю "Экспорт" (скрипт Export):
* Полный Экспорт: скачивает все доступные на сервере транзакции и записывает их на лист экспорта
* Инкрементальный Экспорт: скачивает изменения на сервере с момента последнего экспорта данных и применяет их к листу экспорта
* Подготовить лист изменений: скачивает изменения на сервере с момента последнего экспорта данных и записывает их в отдельный лист изменений
* Применить изменения: применяет изменения с отдельного листа к листу экспорта

Подменю "Импорт" (скрипт Import):
* Полный Импорт: полный импорт передает на сервер все операции с листа импорта, как новые. На текущий момент деактивирован
* Частичный Импорт: передает на сервер только новые операции, а также операции, отмеченные соответствующим флажком
* Отслеживать изменения (скрипт Tracking): включает триггеры отслеживания изменений на листе импорта, выставляющие флажок при изменении существенных данных транзакции
* Снять отслеживание (скрипт Tracking): выключает триггеры отслеживания изменений на листе импорта
* Setup Validation (скрипт Validation): включает проверку данных на соответствие справочникам на листе импорта
* Clear All Validation (скрипт Validation): выключает проверку данных на листе импорта

Подменю "Категории" (скрипт Categories):
* Загрузить: экспорт категорий с сервера на отдельный лист
* Сохранить: импорт категорий с отдельного листа на сервер
* Частично: частичный импорт категорий, отмеченных соответствующим флажком
* Заменить: полная замена категорий на сервере категориями с листа
* Заменить в операциях: замещает одну категорию другой по аналогии с функционалом веб-версии. Исходная категория при этом не удаляется

Подменю "Счета" (скрипт Accounts):
* Загрузить: экспорт счетов с сервера на отдельный лист
* Сохранить: импорт счетов с отдельного листа на сервер
* Частично: частичный импорт счетов, отмеченных соответствующим флажком
* Заменить: полная замена счетов на сервере счетами с листа

Подменю "Места" (скрипт Merchants):
* Загрузить: экспорт мест с сервера на отдельный лист
* Сохранить: импорт мест с отдельного листа на сервер
* Частично: частичный импорт мест, отмеченных соответствующим флажком
* Заменить: полная замена мест на сервере местами с листа
...
Функция "Сохранить" сохраняет изменения на листе целиком и отправляет на сервер полный список категорий независимо от того, вносились ли в них изменения.

Функция "Частично" отправляет на сервер только изменения в операциях, отмеченных флажком "Изменить".

Функция "Заменить" замещает категории на сервере категориями с листа.

Удаление категорий производится через выставление флажка "Удалить" и передачу изменений путём вызова Сохранить/Частично. По неизвестным мне причинам удаление категорий занимает очень много времени, и ответа от сервера вы, скорее всего, не дождётесь. Тем не менее, операция будет выполнена - но в случае с большим количеством категорий это может занять несколько минут или часов. В течение этого времени сервер не считает категории удалёнными; запаситесь терпением и не проводите операций с удаляемыми категориями. Проверить, выполнена ли операция корректно, можно, выгрузив категории с сервера - отправленные на удаление помечаются красным цветом.

Удаление родительской категории приводит также к удалению всех её дочерних категорий. Если в одном запросе с удалением родительских категорий поменять связи дочерних с удаляемыми родительскими, дочерние категории удалены не будут - даже если вы назначите им в родители другую удаляемую категорию.

При удалении категорий ведётся подсчёт удаляемых категорий (включая дочерние), а также затрагиваемых операций. Скрипт предупредит вас, если вы попытаетесь удалить "непустую" категорию.

# Скрипт счетов Accounts.gs
Всё то же самое, что для категорий, только объект другой.

Есть три режима работы - "Сохранить", "Частично" и "Заменить". "Сохранить" обрабатывает все строки на листе счетов (добавляет новые, если ID нет в списке текущих; меняет существующие; удаляет счета, отмеченные флагом "Удалить"), "Частично" - обрабатывает только строки, отмеченные флагом "Изменить". Функция "Заменить" пытается взять структуру с листа счетов, и привести счета на сервере в соответствие с ней, т.е. воссоздать счета с листа, которых нет на сервере, и удалить с сервера те счета, которых нет на листе. Функция "Заменить" определяет, существует счёт или нет, по его ID - т.е. счета с одинаковым названием, но с разными ID для неё разные.
...
Удаление счетов производится через выставление флажка "Удалить" и передачу изменений путём вызова Сохранить/Частично. При удалении счетов ведётся подсчёт затрагиваемых операций. Скрипт предупредит вас, если вы попытаетесь удалить счёт с операциями. Будьте аккуратны, в отличие от обычного удаления операций (в приложении или через скрипт импорта) восстановление операций с удалённого счёта невозможно без бэкапа.



# Изменения
- 06.07.25. Выпущена версия 1.0. Умеет работать с операциями, категориями, счетами и местами.
- 09.07.25. Жаловались, что скрипт категорий зависает при попытке обработать 7000 категорий за раз. Оптимизировал, теперь не зависает. Тестируется режим параллельной отправки нескольких запросов на удаление.

В планах: 
* Добавить бэкапы остальных объектов JSON
* Разобраться с кредитными счетами, их принципом работы и отличиями от остальных
* Вынести больше настроек из скрипта Settings в лист настроек
* Дописать функциональную валидацию, научить её различать категории и теги
* Заменить инициализацию на ленивую в скриптах импорта, экспорта и словарей
* Вывести больше данных в читаемый формат
* Настроить автообновление справочников при импорте
* Настроить автосинхронизацию по расписанию
